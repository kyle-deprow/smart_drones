...
def encode_tasks(takeoff_phrases, move_tasks, move_phrases, shape_tasks, shape_phrases):
  mapping = {task: i for i, task in enumerate(['takeoff'] + move_tasks + shape_tasks)}
  rvalue = {task: {'task': None, 'encoding': None} for task in mapping.keys()}

  # Takeoff tasks
  rvalue['takeoff']['task'] = []
  rvalue['takeoff']['encoding'] = np.zeros(len(mapping))
  rvalue['takeoff']['encoding'][mapping['takeoff']] = 1
  for phrase in takeoff_phrases:
    rvalue['takeoff']['task'] += [phrase]

  for task in move_tasks:
    rvalue[task]['task'] = []
    rvalue[task]['encoding'] = np.zeros(len(mapping))
    rvalue[task]['encoding'][mapping[task]] = 1
    for verb in move_phrases:
      rvalue[task]['task'] += [verb + task]
...
def generate_phrases(json_input, json_output):
  ...
  tasks = encode_tasks(
    json_dict['takeoff_phrases'],
    json_dict['move_tasks'],
...
if __name__ == '__main__':
  ...
  generate_phrases(args.config_file, args.output_file)
